...
class BGAPI2_DECL BufferList {
 public:
    ...
    * 
    *            When a composite buffer is provided, all missing memory segments required for the acquisition
    *            are also allocated when this function is called.
    *            See BGAPI2::Buffer::AddSegment
    * 
    ...
    * \exception Exceptions::ObjectInvalidException The calling object is not valid or
    *            the corresponding datastream is closed.
    * \exception Exceptions::LowLevelException GenTL Producer error. Use IException::GetErrorDescription or check trace
    *            output for more detailed error information.
    */
    ...
    *
    * \exception Exceptions::InvalidParameterException The passed parameter is not a valid Buffer object or
    *            the buffer object is not in the BufferList.
    * \exception Exceptions::ObjectInvalidException The calling object is not valid or the corresponding datastream of
    *            the BufferList is not open.
    * \exception Exceptions::LowLevelException GenTL Producer error. Use IException::GetErrorDescription or check trace
    *            output for more detailed error information.
    */
    ...
    */
    ...
    /**
    * \fn        GetStartedCount
    * \brief     This function delivers the number of Buffer objects which are filled since the stream was opened.
    * \return    bo_uint64 The number of Buffer objects which are filled since the stream was opened.
    * \exception Exceptions::ObjectInvalidException The calling object is not valid.
    * \exception Exceptions::LowLevelException GenTL Producer error. Use IException::GetErrorDescription or check trace
    *            output for more detailed error information.
    */
...
class BGAPI2_DECL Trace {
 public:
    ...
    */
    ...
    /**
    * \fn        IsEnabled
    * \brief     This function delivers the current state of the trace.
    * \return    true if the trace is enabled, otherwise false.
    */
    static bo_bool IsEnabled();
    ...
    * \brief     This function inserts a timestamp difference from the last message into the current message.
    *            This function only takes effect, if the timestamp is activated at all
    *            (see ActivateOutputOptionTimestamp).
...
class BGAPI2_DECL DataStreamEventControl : public EventControl {
 public:
    ...
    *            If no current waiting operation, the next GetFilledBuffer call will be aborted.
    *
...
/**
* \ingroup InterfaceClasses
* \brief The class ImageInfo provide informations about images in a SegmentInfo.
*/
class BGAPI2_DECL ImageInfo {
 public:
    /**
    * \fn GetHeight
    ...
    */
    virtual const bo_uint64 GetHeight() const = 0;
    ...
    */
    virtual const bo_uint64 GetWidth() const = 0;

    /**
    * \fn GetSize
    * \brief  This function delivers the size of the image in bytes.
    * \return bo_uint64 The size of the image in bytes.
    */
    virtual const bo_uint64 GetSize() const = 0;

    /**
    * \fn GetOffset
    * \brief  This function delivers the offset of the image from segment start in bytes.
    * \return bo_uint64 The offset of the image in bytes.
    */
    virtual const bo_uint64 GetOffset() const = 0;

    /**
    * \fn GetSourceID
    * \brief  This function delivers the source id of the image.
    * \return bo_uint64 The source id of the image.
    */
    virtual const bo_uint64 GetSourceID() const = 0;

    /**
    * \fn GetRegionID
    * \brief  This function delivers the region id of the image.
    * \return bo_uint64 The region id of the image.
    */
    virtual const bo_uint64 GetRegionID() const = 0;

    /**
    * \fn GetRegionOffsetX
    * \brief  This function delivers the x offset of the region in pixel.
    * \return bo_uint64 The x offset of the region in pixel.
    */
    virtual const bo_uint64 GetRegionOffsetX() const = 0;

    /**
    * \fn GetRegionOffsetY
    * \brief  This function delivers the y offset of the region in pixel.
    * \return bo_uint64 The y offset of the region in pixel.
    */
    virtual const bo_uint64 GetRegionOffsetY() const = 0;

    /**
    * \fn GetYPadding
    * \brief  This function delivers the number of extra bytes transmitted at the end of the image.
    * \return bo_uint64 The y padding of the image in byte.
    */
    virtual const bo_uint64 GetYPadding() const = 0;

   /**
    * \fn GetXPadding
    * \brief  This function delivers the number of extra bytes transmitted at the end of each line.
    * \return bo_uint64 The x padding of the image in byte.
    */
    virtual const bo_uint64 GetXPadding() const = 0;

    /**
    * \fn GetGroupID
    * \brief  This function delivers the group id of the image.
    * \return bo_uint64 The group id of the image.
    */
    virtual const bo_uint64 GetGroupID() const = 0;

    /**
    * \fn GetPixelFormat
    * \brief  This function delivers the pixel format of the image.
    * \return String The pixelformat of the image.
    */
    virtual const BGAPI2::String GetPixelFormat() const = 0;

    /**
    * \fn GetPayloadType
    * \brief  This function delivers the payload type of the image.
    * 
    ...
    */
    virtual const BGAPI2::String GetPayloadType() const = 0;
};

/**
* \ingroup InterfaceClasses
* \brief The class SegmentInfo provide several informations about the layout and content of a buffer.
*
*        When the SegmentInfo is obtained by a DataStream object the informations are preliminary
*        and may change until a buffer was filled.
*/
class BGAPI2_DECL SegmentInfo {
 public:
    /**
    * \fn SegmentInfo
    * \brief Constructor for creating a SegmentInfo object.
    */
    SegmentInfo();

    /**
    * \fn ~SegmentInfo
    * \brief Destructor for deleting a SegmentInfo object.
    */
    ~SegmentInfo();

   /**
    * \fn GetPosition
    * \brief  This function delivers the index of the segment.
    * \return bo_uint Segment Position in GenDC header descritption
    */
    const bo_uint GetPosition() const;

    /**
    * \fn GetSize
    * \brief  This function delivers the size of the segment in bytes.
    * \return bo_uint64 The size of the segment in bytes.
    */
    const bo_uint64 GetSize() const;

    /**
    * \fn ContainsPreliminaryInfo
    * \brief  This function returns the status if the segment informations are preliminary.
    * 
    *         When the SegmentInfo is obtained by a DataStream object the informations are preliminary
    *         and may change until a buffer was filled.
    * \return bo_bool True if the segment informations is preliminary, otherwise false.
    */
    const bo_bool ContainsPreliminaryInfo() const;

    /**
    * \fn GetImagePresent
    * \brief  This function indicates if the segment contains images or not.
    * \return bo_bool True if the segment contain images, otherwise false.
    */
    const bo_bool GetImagePresent() const;

    /**
    * \fn GetChunkPresent
    * \brief  This function indicates if the segment contains chunk data or not.
    * \return bo_bool True if the segment contain chunk data, otherwise false.
    */
    const bo_bool GetChunkPresent() const;

    /**
    * \fn GetImageCount
    * \brief  This function delivers the count of images contained by the segment.
    * \return bo_uint Count of images contained by the segment.
    */
    const bo_uint GetImageCount() const;

    /**
    * \fn GetImageInfo
    * \brief  This function return an ImageInfo object at the requested index of the segment.
    * \param  iImageIndex index of the image which infos should obtained.
    * \return ImageInfo* Pointer to an ImageInfo object.
    * \exception Exceptions::NotAvailableException The requested index is not available.
    */
    const BGAPI2::ImageInfo* GetImageInfo(bo_uint iImageIndex) const;

 private:
    CSegmentInfoObj* m_pCSegmentInfoObjImpl;
    friend class DataStream;
};

/**
* \ingroup MainClasses
* \brief   This class provides access to image data and some information
*          about the received data (e.g. image size, pixel format).
*
*          Most cameras will transmit one image into one buffer. In this case the function
*          `BGAPI2::Buffer::GetImageCount()` will deliver the value "1".
*          Some cameras can transmit so called GenDC buffers. These self describing buffers are
*          able to hold more than a single image per buffer. Informations about the images can be queried by using
*          for example `BGAPI2::Buffer::GetWidth(image_index)` or `BGAPI2::Buffer::GetHeight(image_index)`.
*          By stating an index the image in the buffer can be selected.
*
*          A GenDC buffer can consist of multiple segments and each segment can consist of
*          multiple components (e.g. images). Multiple components can exist in one segment but one component is
*          never spreaded over multiple segments. The layout of the buffer is a camera specific implementation and
*          is not directly changeable. But the buffer layout may change if camera features/configuration are changed.
*
*          GenDC buffer with 4 segments where the images in segment 1 sharing there memory segment:
*          |-----------|-----------:-----------|-----------|-----------|
*          | Segment 0 |        Segment 1      | Segment 2 | Segment 3 |
*          |    Data   |  Image 0  :  Image 1  |  Image 2  |    Data   |
*          |-----------|-----------:-----------|-----------|-----------|
*
*          To examine the GenDC buffer layout transmitted by a camera `BGAPI2::Datastream::GetSegmentCount()` and
*          and `BGAPI2::Datastream::GetSegmentInfo()` should be used. This function fills an BGAPI2::SegmentInfo object that
*          gives basic informations about the buffer segments and its containing images.
*          Attention: Each buffer segment containing an image can also contain (non image) components.
*
*          A buffer with multiple segments can be created with
*          `BGAPI2::Buffer::Buffer()` and `BGAPI2::Buffer::AddSegment()`
*
*          For more flexibility you can also provide memory for specific buffer segments. Then the data will
*          automatically transferred to the provided memory during acquisition. Not assigned segments will
*          be allocated automatically.
*          Note: Not every camera supports the streaming into buffers with multiple segments.
*
*          This means it is possible to provide only the memory for the segment holding image data and
*          ignore other buffer segments because they will handled by BGAPI.
*/
class BGAPI2_DECL Buffer : public INode, public Events::EventControl {
 public:
    ...
    */
    ...
    /**
    * \fn        AddSegment
    * \brief     This functions adds a user provided memory segment to the buffer.
    *            The segments cannot be removed afterwards, but it is possible to overwrite
    *            the buffer pointer and size for a given index multiple times.
    *
    * \note      If you use this function, the buffer will be regarded as a composite buffer from now on.
    *
    * \exception Exceptions::NotAvailableException The buffer is currently added to a BGAPI2::BufferList
    * \exception Exceptions::InvalidParameterException The segment_buffer pointer is not valid or
    *            the segment_buffer_size is zero.
    * \exception Exceptions::ObjectInvalidException The calling object is not valid.
    */
    void AddSegment(bo_uint segment_index, void* segment_buffer, bo_uint64 segment_buffer_size);
    ...
    */
    ...
    /**
    * \fn        GetMemPtr
    * \brief     This function delivers a pointer to one memory segment of the Buffer object.
    *
    * \param     segment_index The index of the memory segment.
    * \return    void* The pointer to one memory segment of the Buffer object.
    *
    * \exception Exceptions::NotAvailableException For the given index there is no memory allocated.
    *            After adding the Buffer object to a BufferList it is possible that additional
    *            memory segments are allocated internally. Then the given index becomes valid and
    *            the memory pointer can be obtained.
    *            See constructors Buffer(), Buffer(void * pUserObj) and Buffer(bool optimizedSize, void* pUserObj).
    *            See `BGAPI"::Buffer::AddSegment()` for more information.
    * \exception Exceptions::ObjectInvalidException The calling object is not valid.
    * \exception Exceptions::LowLevelException GenTL Producer error. Use IException::GetErrorDescription or check trace
    ...
    */
    void* GetMemPtr(bo_uint segment_index = 0);

    /**
    * \fn        GetMemSize
    * \brief     This function delivers the size of one memory segment of the buffer object.
    *
    * \param     segment_index The index of the memory segment.
    * \return    bo_uint64 The size of the memory segment.
    *
    * \exception Exceptions::NotAvailableException For the given index there is no memory allocated.
    *            After adding the Buffer object to a BufferList it is possible that additional
    *            memory segments are allocated internally. Then the given index becomes valid and
    *            the memory size can be obtained.
    *            See constructors Buffer(), Buffer(void * pUserObj) and Buffer(bool optimizedSize, void* pUserObj).
    *            See `BGAPI"::Buffer::AddSegment()` for more information.
    * \exception Exceptions::ObjectInvalidException The calling object is not valid.
    * \exception Exceptions::LowLevelException GenTL Producer error. Use IException::GetErrorDescription or check trace
    ...
    *            and Buffer(bool optimizedSize, void* pUserObj).
    */
    bo_uint64 GetMemSize(bo_uint segement_index = 0);

    /**
    * \fn         GetSegmentCount
    * \brief      This function returns the number of memory segments that this buffer consist of.
    *
    *             The count is changed when calling AddSegment or by adding the Buffer with BufferList::Add
    * \return     bo_uint The number of segments of this buffer
    * \exception  Exceptions::ObjectInvalidException The calling object is not valid.
    */
    bo_uint GetSegmentCount();

    /**
    * \fn         GetSegmentIndex
    * \brief      Returns the segment index where the image is located.
    * 
    *             This may differ from the result of DataStream::GetSegmentInfo().
    *             Only composite buffers may return values different from 0.
    * \return     bo_uint The segment index where the image is located.
    * \exception  Exceptions::NotAvailableException  The Buffer object is not added to a BufferList,
    *             or does not include image data for the given index.
    * \exception  Exceptions::ObjectInvalidException The calling object is not valid.
    */
    bo_uint GetSegmentIndex(bo_uint image_index);

    /**
    * \fn         IsUserSegment
    * \brief      This function returns true if a segment was provided by the user.
    * \param      segment_index  index of the segment that should be requested if user provided
    * \return     true if the segment was added by the user, otherwise false
    * \exception  Exceptions::ObjectInvalidException The calling object is not valid.
    */
    bo_bool IsUserSegment(bo_uint segment_index);
	...
	*
	*            For segmented buffers this function delivers the timestamp of the first segment in the buffer obtained by the camera.
    *
    *            The basic requirement for this function is that the Buffer object needs valid data.
    * \return    bo_uint64 The timestamp of the buffer.
    * \exception Exceptions::NotAvailableException The Buffer object is not added to a BufferList.
    * \exception Exceptions::ObjectInvalidException The calling object is not valid.
    ...
    * \fn        GetWidth
    * \brief     This function delivers the width of the image in pixel.
    * \param     image_index The index of the image within the buffer.
    * \return    bo_uint64 The width of the image in pixel.
    * \exception Exceptions::NotAvailableException The Buffer object is not added to a BufferList,
    *            or does not include image data for the given index.
    * \exception Exceptions::ObjectInvalidException The calling object is not valid.
    ...
    *            output for more detailed error information.
    */
    bo_uint64 GetWidth(bo_uint image_index = 0);
    ...
    *            output for more detailed error information.
    */
    bo_uint64 GetHeight(bo_uint image_index = 0);
    ...
    *            output for more detailed error information.
    */
    bo_uint64 GetXOffset(bo_uint image_index = 0);
    ...
    *            output for more detailed error information.
    */
    bo_uint64 GetYOffset(bo_uint image_index = 0);
    ...
    *            output for more detailed error information.
    */
    bo_uint64 GetXPadding(bo_uint image_index = 0);
    ...
    *            output for more detailed error information.
    */
    bo_uint64 GetYPadding(bo_uint image_index = 0);

    /**
    * \fn        GetFrameID
    * \brief     This function delivers a sequentially incremented number of the image. If more than one image is contained
    *            in the buffer, then each image in this buffer has the same frame id.
    *
    ...
    */
    ...
    /**
    * \fn        GetImagePresent
    * \brief     This function delivers true, if the Buffer object contains at least one Image or Jpeg data.
    * 
    * \return    bo_bool true, if at least one image is present in the buffer.
    * \exception Exceptions::ObjectInvalidException The calling object is not valid.
    * \exception Exceptions::LowLevelException GenTL Producer error. Use IException::GetErrorDescription or check trace
    *            output for more detailed error information.
    */
    ...
    /**
    * \fn        GetImageCount
    * \brief     This function delivers the number of images contained in the buffer.
    * 
    * \return    bo_uint The number of images in the buffer.
    * \exception Exceptions::ObjectInvalidException The calling object is not valid.
    * \exception Exceptions::LowLevelException GenTL Producer error. Use IException::GetErrorDescription or check trace
    *            output for more detailed error information.
    */
    bo_uint GetImageCount();

    /**
    * \fn        GetImageData
    * \brief     This function delivers a pointer to the image data.
    * 
    * \param     image_index The index of the image within the buffer.
    ...
    *            output for more detailed error information.
    */
    void* GetImageData(bo_uint image_index = 0);

    /**
    * \fn        GetImageOffset
    * \brief     This function returns the offset of the image or jpeg data
    *            relative to the beginning of the memory segment the image or
    *            Jpeg data resides in.
    * \note      Use GetImageData instead to retrieve a pointer to the image data.
    *
    * \param     image_index The index of the image within the buffer.
    * \return    bo_uint64 The offset of the image relative to the start of the corresponding
    *            memory segment of the buffer.
    * \exception Exceptions::NotAvailableException The Buffer object is not added to a BufferList,
    *            or does not include image data for the given index.
    * \exception Exceptions::ObjectInvalidException The calling object is not valid.
    */
    bo_uint64 GetImageOffset(bo_uint image_index = 0);
    ...
    *
    * \param     image_index The index of the image within the buffer.
    * \return    bo_uint64 The length of the image in bytes.
    * \exception Exceptions::NotAvailableException The Buffer object is not added to a BufferList,
    *            or does not include image data for the given index.
    * \exception Exceptions::ObjectInvalidException The calling object is not valid.
    */
    bo_uint64 GetImageLength(bo_uint image_index = 0);
    ...
    *
    *            See the payload type definitions in bgapi2_types.h (BGAPI2_PAYLOADTYPE_xxx).
    * \note      If the Buffer object is not added to a BufferList or does not include valid data
    *            then the function returns BGAPI2_PAYLOADTYPE_UNKNOWN.
    *
    * \return    String The payload type of the Buffer object.
    * \exception Exceptions::ObjectInvalidException The calling object is not valid.
    ...
    */
    ...
    /**
    * \fn        GetImageType
    * \brief     This function delivers the payload type of the image in the Buffer object.
    *
    *            See the payload type definitions in bgapi2_types.h (BGAPI2_PAYLOADTYPE_xxx).
    *
    * \param     image_index The index of the image within the buffer.
    * \return    String The payload type of the image.
    * \exception Exceptions::NotAvailableException The Buffer object is not added to a BufferList,
    *            or does not include image data for the given index.
    * \exception Exceptions::ObjectInvalidException The calling object is not valid.
    * \exception Exceptions::LowLevelException GenTL Producer error. A possible reason for this exception is a
    ...
    *            detailed error information.
    */
    String GetImageType(bo_uint image_index = 0);

    /**
    * \fn        GetPixelFormat
    * \brief     This function delivers the pixel format of the image in the Buffer object.
    *
    * \param     image_index The index of the image within the buffer.
    * \return    String The pixelformat of the image.
    * \exception Exceptions::NotAvailableException The Buffer object is not added to a BufferList,
    ...
    *            detailed error information.
    */
    String GetPixelFormat(bo_uint image_index = 0);
    ...
    *            linescan images.
    */
    bo_uint64 GetDeliveredImageHeight(bo_uint image_index = 0);

    /**
    * \fn        GetSourceID
    * \brief     This function delivers the unique source identifier of the corresponding image.
    *            A source can be a sensor or preprocessing module from a camera.
    *
    * \param     image_index The index of the image within the buffer.
    * \return    bo_uint64 The source id of the image.
    * \exception Exceptions::NotAvailableException The Buffer object is not added to a BufferList,
    ...
    *            detailed error information.
    */
    bo_uint64 GetSourceID(bo_uint image_index = 0);

    /**
    * \fn        GetRegionID
    * \brief     This function delivers the unique region identifier of the corresponding image.
    *
    * \param     image_index The index of the image within the buffer.
    * \return    bo_uint64 The region id of the image.
    * \exception Exceptions::NotAvailableException The Buffer object is not added to a BufferList,
    ...
    *            detailed error information.
    */
    bo_uint64 GetRegionID(bo_uint image_index = 0);

    /**
    * \fn        GetGroupID
    * \brief     This function delivers the unique group identifier of the corresponding image.
    *            Each image in the buffer with the same group id logically belong together.
    *
    * \param     image_index The index of the image within the buffer.
    * \return    bo_uint64 The group id of the image.
    * \exception Exceptions::NotAvailableException The Buffer object is not added to a BufferList,
    ...
    *            detailed error information.
    */
    bo_uint64 GetGroupID(bo_uint image_index = 0);
...
class BGAPI2_DECL DataStream : public INode, public Events::DataStreamEventControl {
 public:
    ...
    */
    ...
    /**
    * \fn        GetSegmentCount
    * \brief     This function delivers the number of segments that the datastream payload can be split into.
    *            Every segment is than transfered in a separate memory segment during acquisition.
    *
    *            This functions mainly use case is when a subset of the payload needs to be transfered
    *            somewhere else in memory than the rest.
    *            E.g. Image data separated from chunk or other data
    * 
    * \note      Changing device features may change the resulting SegmentCount.
    * 
    *            The exact contents of every segment is based on the current configuration of the device.
    *            the preliminary contents of each segment can be queried with GetSegmentInfo
    * \return    bo_uint number of distinct memory transfers.
    * \exception Exceptions::NotInitializedException The DataStream object is not open.
    * \exception Exceptions::ObjectInvalidException The calling object is not valid.
    * \exception Exceptions::LowLevelException GenTL Producer error. Use IException::GetErrorDescription or check trace
    *            output for more detailed error information.
    * \see       DataStream::GetSegmentInfo
    */
    bo_uint GetSegmentCount();

    /**
    * \fn        GetSegmentInfo
    * \brief     This function delivers preliminary informations of a requested segement transfered on the datastream.
    * 
    *            If this function is used with a Non GenDC device a segment info is generated by the current camera settings
    *            for width, height and pixelformat. The other informations will be 0.
    * 
    * \note      Changing device features may change the resulting SegmentInfo.
    * 
    * \param     iSegmentIndex index of the segment which infos should obtained.
    * \param     pSegmentInfo  pointer to SegmentInfo object where the informations copied to.
    * \exception Exceptions::NotInitializedException The DataStream object is not open.
    * \exception Exceptions::ObjectInvalidException The calling object is not valid or
    *            the pSegmentInfo object is not valid.
    * \exception Exceptions::InvalidParameterException The segment_info pointer is not valid.
    * \exception Exceptions::NotAvailableException The requested index is currently not available.
    * \exception Exceptions::LowLevelException GenTL Producer error. Use IException::GetErrorDescription or check trace
    *            output for more detailed error information.
    */
    void GetSegmentInfo(bo_uint iSegmentIndex, SegmentInfo* pSegmentInfo);
...
class BGAPI2_DECL Device : public INode, public Events::DeviceEventControl {
 public:
    ...
    * \exception Exceptions::ObjectInvalidException The calling object is not valid.
    * \exception Exceptions::InvalidParameter matrix missing.
    * \exception Exceptions::NotImplemented color matrix not supported.
    * \exception Exceptions::Error Error while reading color matrix.
    */
    void GetColorMatrix(double matrix[3][3]);

    /**
    * \fn        SetColorMatrix
    * \brief     This function directly writes the color matrix to the camera and depends on several camera settings.
    * \brief     Please make sure that all features which change the color matrix automatically are switch off.
    * \param     matrix A 3x3 matrix.
    * \exception Exceptions::ObjectInvalidException The calling object is not valid.
    * \exception Exceptions::InvalidParameter matrix missing.
    * \exception Exceptions::NotImplemented color matrix not supported.
    ...
    * \param     comportname  os specific name of pc comport to connect serial feature to comport
    *            empty string to deconnect serial features from comport
    * \exception Exceptions::NotAvailableException       Missing cam genicam feature(s)
    * \exception Exceptions::NotImplementedException     Can't configure serial port
    * \exception Exceptions::ObjectInvalidException      Internal object is invalid
    * \exception Exceptions::InvalidParameterException   Error on open com port
    * \exception Exceptions::LowLevelException           Error on configure com port
    */
...
class BGAPI2_DECL ImageProcessor : public INode {
 public:
    ...
    */
    ...
    /**
    * \fn        CreateImage()
    * \brief     This function creates a 1x1 Mono8 Image object.
    ...
    * \param     pBuffer The destination buffer - please do not pass a BGAPI2::Buffer object.
    ...
    * \param     matrix A 3x3 matrix.
    * \exception Exceptions::InvalidParameterException matrix missing.
    * \exception Exceptions::ObjectInvalidException The calling object is not valid.
    */
    void GetColorMatrix(double matrix[3][3]);

...
extern "C"
{
    ...
    //   runtime. If memory leak detection is already disabled, then calling
    ...
    //    detection is enabled for other threads, then it will remain enabled
    //    for those other threads. It was designed to work this way to insulate
    //    you, the programmer, from having to ensure thread synchronization when
    //    calling VLDEnable() and VLDDisable(). Without this, calling these two
    //    functions unsynchronized could result in unpredictable and unintended
    //    behavior. But this also means that if you want to disable memory leak
    //    detection process-wide, then you need to call this function from every
    ...
    // VLDEnable - Enables Visual Leak Detector's memory leak detection at
    // runtime.
    //   If memory leak detection is already enabled, which it is by default,
    //   then calling this function has no effect.
    //
    //  Note: In multithreaded programs, this function operates on a per-thread
    //    basis. In other words, if you call this function from one thread, then
    //    memory leak detection is only enabled for that thread. If memory leak
    //    detection is disabled for other threads, then it will remain disabled
    //    for those other threads. It was designed to work this way to insulate
    //    you, the programmer, from having to ensure thread synchronization when
    //    calling VLDEnable() and VLDDisable(). Without this, calling these two
    //    functions unsynchronized could result in unpredictable and unintended
    //    behavior. But this also means that if you want to enable memory leak
    //    detection process-wide, then you need to call this function from every
    //    thread in the process.
    //
    //  Return Value:
    ...
    // VLDGlobalDisable - Disables Visual Leak Detector's memory leak detection
    // at
    ...
    // VLDGetThreadLeaksCount - Return thread memory leaks count to the
    // execution point.
    //
    // threadId: thread Id.
    //
    ...
    // VLDRefreshModules - Look for recently loaded DLLs and patch them if
    // necessary.
    //
    //  Return Value:
    //
    ...
    // VLDSetOptions - Update the report options via function call rather than
    // INI file.
    //
    ...
    //
    // maxDataDump: maximum number of user-data bytes to dump for each leaked
    // block.
    //
    // maxTraceFrames: maximum number of frames per stack trace for each leaked
    // block.
    //
    //  Return Value:
    //
    ...
    // VLDSetModulesList - Set list of modules included/excluded in leak
    // detection depending on parameter "includeModules".
    //
    // modules: list of modules to be forcefully included/excluded in leak
    // detection.
    //
    ...
    //    None.
    //
    __declspec(dllimport) void VLDSetModulesList(const wchar_t *modules,
                                                 VLD_BOOL includeModules);

    // VLDGetModulesList - Return current list of included/excluded modules
    // depending on flag VLD_OPT_TRACE_INTERNAL_FRAMES.
    //
    // modules: destination string for list of included/excluded modules
    ...
    //
    __declspec(dllimport) VLD_BOOL VLDGetModulesList(wchar_t *modules,
                                                     VLD_UINT size);

    // VLDSetReportOptions - Update the report options via function call rather
    // than INI file.
    //
    ...
    //    None.
    //
    __declspec(dllimport) void VLDSetReportOptions(VLD_UINT option_mask,
                                                   const wchar_t *filename);

    // VLDSetReportHook - Installs or uninstalls a client-defined reporting
    // function by hooking it
    ...
    // VLDResolveCallstacks - Performs symbol resolution for all saved extent
    // CallStack's that have been tracked by Visual Leak Detector. This function
    // is necessary for applications that dynamically load and unload modules,
    // and through which memory leaks might be included. If this is NOT called,
    // stack traces may have stack frames with no symbol information. This
    // happens because the symbol API's cannot look up symbols for a binary /
    // module that has been unloaded from the process.
    //
    //  Return Value:
    //
